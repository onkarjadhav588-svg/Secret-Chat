...
function friendlyAuthError(err) {
  const code = err.code || "";
  if (code.includes("email-already-in-use")) return "This account already exists, try Login instead";
  if (code.includes("invalid-credential") || code.includes("wrong-password") || code.includes("user-not-found")) return "Incorrect password";
...
msgInput.addEventListener("keypress", (e) => {
  if (e.key === "Enter") {
    e.preventDefault(); // stops mobile browsers from treating this as a form-submit and closing the keyboard
    sendBtn.click();
  }
});
...
sendBtn.onclick = async () => {
  ...
  clearTypingStatus();
  msgInput.focus(); // keep the mobile keyboard open after sending
};
